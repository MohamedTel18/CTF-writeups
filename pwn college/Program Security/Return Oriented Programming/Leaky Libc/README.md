# Leaky Libc Exploit Explanation
...
- Creates a 72-byte padding buffer (64 'A's + 8 'B's) to overflow the stack

**Payload Construction:**
```
[padding] → [ret] → [pop rdi] → [system@GOT] → [puts PLT] → [main]
```

**Execution:**
1. Overwrites return address with ROP gadgets
2. Sets `rdi = system@GOT` (address of system in Global Offset Table)
3. Calls `puts()` via PLT to print the leaked address
4. Returns to `main` to allow a second overflow

**Result:** Leaks the runtime address of `system()` in libc, allowing calculation of the libc base address.
